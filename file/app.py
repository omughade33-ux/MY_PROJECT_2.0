...
conn = psycopg2.connect(
    ...
    "postgresql://postgres.mukvklawjdmetyrchxgn:om sai ram 1234@aws-1-ap-south-1.pooler.supabase.com:6543/postgres"
...
def register():
    try:
        ...
        # For transporter, GST should be NULL
        gst_value = gst if role == 'company' and gst else None
        
        # Insert user
        cursor.execute("""INSERT INTO users (name, email, password_hash, role, phone, gst_number, is_verified, created_at) 
...
def get_load(load_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""SELECT l.*, u.name as company_name, u.phone as company_phone, u.gst_number
                 FROM loads l 
                 JOIN users u ON l.company_id = u.id
                 WHERE l.id = %s""", (load_id,))
...
def place_bid(load_id):
    ...
    cursor.execute("SELECT id FROM bids WHERE load_id = %s AND transporter_id = %s", (load_id, session['user_id']))
    if cursor.fetchone():
        conn.close()
    ...
    created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cursor.execute("INSERT INTO bids (load_id, transporter_id, bid_amount, status, created_at) VALUES (%s, %s, %s, %s, %s)",
...
def get_bids_for_load(load_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""SELECT b.*, u.name as transporter_name, u.phone as transporter_phone
                 FROM bids b 
                 JOIN users u ON b.transporter_id = u.id
                 WHERE b.load_id = %s
                 ORDER BY b.bid_amount ASC""", (load_id,))
...
def accept_bid(bid_id):
    ...
    cursor.execute("""SELECT b.*, l.company_id, l.id as load_id
                 FROM bids b 
...
def dashboard():
    ...
    if role == 'company':
        cursor.execute("""SELECT l.*, COUNT(b.id) as bids_count
                     FROM loads l
        ...
        cursor.execute("""SELECT b.*, l.goods_name, l.pickup_location, l.delivery_location, l.weight
                     FROM bids b
                     JOIN loads l ON b.load_id = l.id
...
if __name__ == '__main__':
    print("=" * 50)
    print("  🚛 CargoConnect Backend Server (MySQL)")
    print("  📍 Running at: http://localhost:5000")
    print("  📝 API Base: http://localhost:5000/api")
    print("=" * 50)
    print("\n🔐 Test Accounts:")
    print("  Company: rahul@shipping.com / password123")
    print("  Transporter: amit@transport.com / password123")
    print("=" * 50)
